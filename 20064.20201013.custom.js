//~~tv:20064.20201013
//~~tc: removing IAB v1.1 compliance

// MODIFIED to use /event and simply the payload appropriately for that endpoint, changed marked with (CUSTOM) for convenience
...
(function (id, loader, u) {
  ...
  // (CUSTOM) - without this override, DLE will break
  u.visitor_service_override = "https://visitor-service.tealiumiq.com"; // setting this property will override the domain used in the data layer enrichment call; the rest of the URL will build normally (pathname with the account and profile, query parameters, etc.)
  ...
  // (CUSTOM) Server list override, use /event endpoint to avoid setting TAPID cookie (allows visitor switching on shared devices)
  // u.server_list = u.tag_config_server.split("|"); // OLD LOGIC
  ...
  u.send = function(a,b) {
    ...
    if(u.ev[a] || typeof u.ev["all"] !== "undefined"){
      ...
      ##UTEXTEND##
        ...
        // (CUSTOM) Remove form, /event needs a simpler payload
        // if (window.FormData) {
          // modern browsers
          // eslint-disable-next-line no-inner-declarations
          function postData(server_index, enrichment_polling, enrichment_polling_delay) {
            ...
            u.server_list[server_index] = infixParameters(u.server_list[server_index], u.tc_string);
            xhr.open("post", u.server_list[server_index], true);
            xhr.withCredentials = true;
            
            // (CUSTOM) Update the profile to make sure it goes to the right place
            u.data.data.tealium_profile = u.profile;
            
            // (CUSTOM) use only part of the information for the sake of visitor switching
            // json_string = JSON.stringify(u.data); // OLD LOGIC
    ...
    // (CUSTOM) no need for this because we're not using forms
    /*
        } else {
